// ---
{
    ...
    // ```tsx
    // // 함수 정의
    const sumAllNumbers = function (...arr: Array<number>[]): number[] {
        return arr.map(arr => arr.reduce((acc, cur) => acc + cur, 0));
    }
...

{
    ...
    // ```tsx
    // // 함수 정의
    // arr은 동적으로 변하니깐 위에 배열은 건들지 말라는 조건에 따라, arr[0], arr[1]만 사용해서 인덱스별 곱셈만 수행
    const multiplyArrays = function (...arr: Array<number>[]): number[] {
        if (!arr[0] || !arr[1]) return [];
        const minLen = Math.min(arr[0]?.length ?? 0, arr[1]?.length ?? 0);
        return Array.from(
            { length: minLen },
            (_, index) => (arr[0]?.[index] ?? 0) * (arr[1]?.[index] ?? 0)
        );
    }
...

{
    // ```tsx
    // // 함수 정의
    const addStringAndNumber = (value: string, value2: number): string =>
        `${value}${value2}`;
...
// ---
{
    ...
    // ```tsx
    // // 함수 정의
    const compareValues = (value: number, value2: string): string => {
        if (typeof value === 'number' && typeof value2 === 'string') {
            return value > value2.length ? "Number is larger" : "String is larger"
        }
