...
// ```tsx
{
    const doubleOrLength = (value: number | string): number => {
        ...
        // 이 부분은 타입이 number | string이므로 사실상 도달하지 않음
        return 0;
    }
...
// **문제:**
// 두 값을 받아서 그 중 더 큰 값을 반환하는 함수 `maxValue`를 작성하세요. 입력 값은 `number` 또는 `string`일 수 있으며, 
// 숫자일 경우 그대로 반환하고, 문자열일 경우 길이가 더 긴 문자열을 반환합니다. 만약 두 값이 같으면 그 값을 반환하세요.

// ```tsx
{
    const maxValue = (value: string | number, value2: string | number): string | number | undefined => {
        if (typeof value === "number" && typeof value2 === "number") {
            return value > value2 ? value : value2
        }
        if (typeof value === "string" && typeof value2 === "string") {
            return value.length > value2.length ? value : value2
        }
